...
const ListAddFriend = () => {
  ...
  const handleAcceptInvite = async (invitation) => {
    ...
    const response = await axios.post(GET_ALL_USER + userInfo?.id, invitation);
    ...
    try {
      ...
      dispatch({ type: reducerCases.SET_RECEIVE_INVITATION, receive: updatedReceivedInvitations ? updatedReceivedInvitations : [] })
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  }
  ...
  const handleCancelInvite = async (invitation) => {
    ...
    try {
      ...
      dispatch({ type: reducerCases.SET_SENT_INVITATION, send: updatedSendedInvitations ? updatedSendedInvitations : [] })
  ...
  return (
    <div className="px-3" style={{ backgroundColor: 'white', height: '100vh' }}>
      ...
      >
        <u>Friend Requests Sent</u> <span>({sentInvitations?.length || 0})</span>
      </div>
