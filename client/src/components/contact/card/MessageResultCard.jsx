...
const MessageResultCard = ({
  ...
  } else if (filteredMessagesByName) filteredMessages = filteredMessagesByName;
        ...
        filteredMessages.map(
          (message, index) =>
            (message.type.includes("text") ||
              message.type.includes("reply")) && (
              <div
                key={index}
                className="tw-pl-4 tw-flex tw-justify-start tw-items-center tw-border-b-2 tw-border-slate-200 tw-break-words"
              >
                ...
                <div className="tw-w-7/12 tw-mt-3 tw-text-sm tw-pl-2">
                  <p className="tw-font-bold tw-text-[18px] ">
                    {message.senderName}
                  </p>
                  <p>{message.content}</p>
                </div>
