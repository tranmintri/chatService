...
const RemoveMessageModal = ({ showModal, handleCloseModal, removeMessage }) => {
    ...
    const handleRemove = async () => {
        if (selectedOption === 0) {
            await axios.delete(CHAT_API + currentChat.chatId + "/messages/" + removeMessage.messageId)
            const updatedMessages = messages.filter(msg => msg.messageId !== removeMessage.messageId);
        ...
        if (selectedOption === 1) {
            await axios.put(CHAT_API + currentChat.chatId + "/messages/" + removeMessage.messageId)
            handleCloseModal()
            const updatedMessages = [...messages];

            // Find the message by messageId and update its status
            const messageIndex = updatedMessages.findIndex(msg => msg.messageId === removeMessage.messageId);
            if (messageIndex !== -1) {
                // Update the status of the message at the found index
                updatedMessages[messageIndex] = { ...updatedMessages[messageIndex], status: 'removed' };
    ...
    return (
        <Modal show={showModal} onHide={handleCloseModal} backdrop="static">
            ...
            <Modal.Body style={{ background: '#abcedb' }}>
                {isTimestampWithinOneMinute(removeMessage.timestamp) && removeMessage.senderId == userInfo?.id && (
                    <div>
                        <Form.Check
                            type="radio"
